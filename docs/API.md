...
**The `directions` layer follows its routes.** Like [`mask`](#mask), it is a system layer present in every atlas (`index.atlas.json` defines it; it is injected into any atlas that doesn't — see `SYSTEM_LAYERS` in `js/map-init.js`) and it is only on the map while it holds something: the first route drawn switches it on, and the last one removed switches it off again, so it never sits empty in the layer stack or in a shared URL. Switching it off — its toggle in the layer drawer, or **Remove** in the layer stack strip — discards the routes drawn in it rather than hiding them, along with the waypoint pins the routing placed itself; a pin that was already on the map when a route adopted it stays behind as an ordinary marker. Routes restored from a `route-<rid>:` link have a layer each and behave the same way.
...
**Adding the same source via the map creator:** `map-creator.html` accepts the equivalent full URL pasted directly into the URL box, and auto-fills title/attribution/style via `js/layer-source-resolver.js` — the same module this shorthand dispatcher pulls from (see Implementation below):
...
**Click inspection**: Mapbox GL JS's `Map#queryRasterValue()` only supports `raster-array` sources (a special numeric-tile format) — none of this app's raster types use it, since their tiles are plain colored images with the classification already applied server-side. Instead, `js/raster-pixel-inspector.js` reads the actual rendered pixel color at the clicked point via `WebGLRenderingContext#readPixels`. Only one composite color is sampled — the final blended pixel — so there's no way to tell which stacked layer actually painted it; `_inspectRasterPixel` (`js/map-marker-manager.js`) handles this by trying every active raster layer's own `legendMap` against that same sampled color, topmost layer first, and using whichever one's nearest class comes within a max-distance threshold. This means a legend-less overlay (e.g. a semi-transparent regional plan raster) sitting above a classified layer (e.g. ESA WorldCover) doesn't make the layer beneath it permanently uninspectable.

This click inspection isn't limited to layers that declare a `legendMap` — clicking **any** active `tms`/`wmts`/`wms`/`cog` layer with no vector feature under it samples the pixel and labels it. If no active layer's `legendMap` matches closely enough (or none has one), the raw color is shown as a `#rrggbb` hex string instead, attributed to the topmost active raster layer since that's the one actually visible at the click. This requires the map to be constructed with `preserveDrawingBuffer: true` (see `index.html`) — without it, the WebGL drawing buffer is cleared before it can be read back.

**Caveat**: the composited alpha only reflects the *final* canvas, not whether the topmost layer itself painted anything at this specific pixel. A raster with real per-pixel transparency (a cloud-masked mosaic, a georeferenced scan that doesn't cover the whole tile) reads back fully opaque wherever an opaque basemap or another raster sits beneath its own gaps — so its hex fallback there is actually reporting whatever's underneath, not its own (nonexistent) data at that point. There's no way to tell this apart from a genuinely opaque reading without sampling each layer in isolation, which this doesn't do.

If the inspected layer has a `legendImage`, it's shown (click-to-expand into a full-screen view) inside the click badge's expanded details alongside the class/color label — see `_buildLegendImageHTML`/`_showImageLightbox` in `js/map-marker-manager.js`.

Because the match works on the final composited pixel, a `legendMap` only reliably labels clicks while the color it's matching against hasn't been pulled too far off by another raster blended on top of it — a heavily opaque, uncorrelated layer stacked above can still shift the composite far enough that no legend matches and the click falls back to a hex color.
