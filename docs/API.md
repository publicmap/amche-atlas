...
This hex fallback is suppressed (no badge shown at all) when a `basemap`-tagged raster layer (e.g. `mapbox-satellite`) is active anywhere beneath the topmost non-basemap layer. The composited alpha only reflects the *final* canvas, not whether the topmost layer painted anything at this specific pixel — a raster with partial coverage (a cloud-masked mosaic, a georeferenced scan that doesn't cover the whole tile) reads back fully opaque wherever an opaque basemap sits beneath its own transparent areas, so without this check the fallback would mislabel the basemap's color as the overlay's own reading.
